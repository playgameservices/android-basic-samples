...
class PlayScene : public Scene {
    ...
    protected:
        ...
        // how many steps of the "collect bonus" achievement are pending to send
        int mCollectAchPendingSteps;

        // how many steps of the "crash" achievement are pending to send
        int mCrashAchPendingSteps;

        // keep track of which achievements we have unlocked, to avoid making
        // repeated requests
        bool mUnlocked[IDX_ACH_COUNT];

        // last time when we sent the incremental achievements
        float mLastIncAchSendTime;
        ...
        // unlock any achievements that have to do with reaching a given level
        void UnlockLevelBasedAchievements();

        // unlock the given achievement
        void UnlockAchievement(int index);

        // sends incremental achievements, if necessary. If force is true, sends
        // then regardless of how long it has been since they were sent.
        void SendIncrementalAchievements(bool force);
