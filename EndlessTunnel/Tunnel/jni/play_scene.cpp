...
PlayScene::PlayScene() : Scene() {
    ...
    mCollectAchPendingSteps = 0;
    mCrashAchPendingSteps = 0;

    memset(mUnlocked, 0, sizeof(mUnlocked));

    mLastIncAchSendTime = Clock();
    ...
    const char *savePath = OurActivity_GetFilesDir();
...
void PlayScene::LoadProgress() {
    ...
    if (!BGNActivity_IsSignedIn()) {
        LOGD("No cloud save available because we are not signed in.");
        mUseCloudSave = false;
    } else {
        cloudData = OurActivity_GetCloudData();
        LOGD("Data loaded from cloud: %d", cloudData);
        if (cloudData == OURACTIVITY_CLOUD_FAILED) {
            LOGD("Cloud load failed. Falling back to using local data.");
            mUseCloudSave = false;
        } else {
            LOGD("Cloud data loaded successfully.");
            mUseCloudSave = true;

            // if the locally saved progress is better, use that.
            if (cloudData > mSavedCheckpoint) {
                mSavedCheckpoint = cloudData;
                LOGD("Since cloud > local, updated local level to %d", mSavedCheckpoint);
            } else if (cloudData < mSavedCheckpoint) {
                LOGD("Since cloud < local, updating cloud to match local: %d", mSavedCheckpoint);
                OurActivity_SaveState(mSavedCheckpoint);
            }
        }
    }
...
void PlayScene::SaveProgress() {
    ...
    if (mUseCloudSave) {
        ...
        OurActivity_SaveState(mDifficulty);
    } else {
...
void PlayScene::DoFrame() {
    ...
    // If a sign in result is pending, then we should put up a wait screen
    // while that gets sorted out. This is important because while the sign in process
    // is pending, we can't make any API calls to Play Games, so if we started playing
    // before the process was complete, we might try to unlock an achievement or
    // post to a leaderboard before the API is ready. Remember that every time the app
    // goes into the background and is brought back, it has to sign in again, so this
    // might happen in the middle of gameplay, not just at startup.
    if (BGNActivity_IsInProgress()) {
        mTextRenderer->SetFontScale(SIGN_FONT_SCALE);
        mTextRenderer->RenderText(S_PLEASE_WAIT, mgr->GetScreenAspect() * 0.5f, 0.0f);
        return;
    }
    ...
    if (!mShowedHowto && mDifficulty == 0) {
        mShowedHowto = true;
        ShowSign(OurActivity_IsJoystickPresent() ? S_HOWTO_WITH_JOY :
                S_HOWTO_WITHOUT_JOY, SIGN_DURATION);
    }
    ...
    // send incremental achievements, if it's time
    ...
    if (mLives <= 0 && Clock() > mGameOverExpire) {
        ...
        // increment the "frequent player" achievement
        BGNActivity_IncrementAchievement(ACH_INC_PLAY, 1);
    }
...
void PlayScene::DetectCollisions(float previousY) {
    ...
    if (o->grid[col][row]) {
        ...
        if (mLives > 0) {
            ...
            BGNActivity_SubmitScore(LEADERBOARD_ID, (long)GetScore());

            // submit incremental achievements, since game ended
        ...
        if (mLastCrashSection == mFirstSection) {
            // double crash
            UnlockAchievement(IDX_ACH_DOUBLE_CRASH);
        } else {
            // mark this section as being the last crash section
            mLastCrashSection = mFirstSection;
        }

        // add one step to the "crash test dummy" achievement
        mCrashAchPendingSteps++;
    } else if (row == o->bonusRow && col == o->bonusCol) {
        ShowSign(S_GOT_BONUS, SIGN_DURATION_BONUS);
        o->DeleteBonus();
        AddScore(BONUS_POINTS);
        mBonusInARow++;
        mCollectAchPendingSteps++;

        // unlock achievement?
        if (mBonusInARow >= 10) {
            LOGD("%d bonus in a row. Unlocking achivement.", mBonusInARow);
            UnlockAchievement(IDX_ACH_TEN_IN_A_ROW);
            mBonusInARow = 0;
        }
        ...
        // show encouragement toasts as appropriate
        OurActivity_ShowEncouragementToasts(score);
    } else if (o->HasBonus()) {
        // player missed bonus!
        mBonusInARow = 0;
    }

    // check for the ACH_THROUGH_HOLE achievement.
    // is it a hole? (that is, an empty cell surrounded by obstacle cells)
    bool isHole = row > 0 && row < OBS_BOX_SIZE - 1 && col > 0 && col < OBS_BOX_SIZE - 1 &&
            o->grid[col-1][row] && o->grid[col+1][row] && o->grid[col][row-1] &&
            o->grid[col][row+1];

    if (isHole) {
        UnlockAchievement(IDX_ACH_THROUGH_HOLE);
    }
    ...
    if (!o->grid[col][row]) {
        ...
        if (isCloseCall) {
            UnlockAchievement(IDX_ACH_CLOSE_CALL);
        }
    }
}

void PlayScene::UnlockLevelBasedAchievements() {
    if (mDifficulty >= LEVEL_E) {
        UnlockAchievement(IDX_ACH_REACH_LEVEL_E);
    }
    if (mDifficulty >= LEVEL_D) {
        UnlockAchievement(IDX_ACH_REACH_LEVEL_D);
    }
    if (mDifficulty >= LEVEL_C) {
        UnlockAchievement(IDX_ACH_REACH_LEVEL_C);
    }
    if (mDifficulty >= LEVEL_B) {
        UnlockAchievement(IDX_ACH_REACH_LEVEL_B);
    }
    if (mDifficulty >= LEVEL_A) {
        UnlockAchievement(IDX_ACH_REACH_LEVEL_A);
    }

    int score = GetScore();
    if (score > SCORE_A && mLastCrashSection < 0) {
        UnlockAchievement(IDX_ACH_PERFECT_SCORE_A);
    }
    if (score > SCORE_B && mLastCrashSection < 0) {
        UnlockAchievement(IDX_ACH_PERFECT_SCORE_B);
    }
    if (score > SCORE_C && mLastCrashSection < 0) {
        UnlockAchievement(IDX_ACH_PERFECT_SCORE_C);
    }
}
...
void PlayScene::UnlockAchievement(int index) {
    static const char* ach_ids[] = IDX_TO_ACHIEVEMENT;
    if (!mUnlocked[index] && BGNActivity_IsSignedIn()) {
        LOGD("Unlocking achievement %d, %s", index, ach_ids[index]);
        BGNActivity_UnlockAchievement(ach_ids[index]);
        mUnlocked[index] = true;
    }
}

void PlayScene::SendIncrementalAchievements(bool force) {
    if (BGNActivity_IsSignedIn() && (force || Clock() - mLastIncAchSendTime >
            INC_ACH_SEND_INTERVAL)) {
        if (mCollectAchPendingSteps > 0) {
            BGNActivity_IncrementAchievement(ACH_INC_COLLECT_BONUS, mCollectAchPendingSteps);
            mCollectAchPendingSteps = 0;
        }
        if (mCrashAchPendingSteps > 0) {
            BGNActivity_IncrementAchievement(ACH_INC_CRASH, mCrashAchPendingSteps);
            mCrashAchPendingSteps = 0;
        }
        mLastIncAchSendTime = Clock();
    }
...
void PlayScene::HandleMenu(int menuItem) {
    switch (menuItem) {
        ...
        case MENUITEM_RESUME:
            ...
            OurActivity_ResetEncouragementToasts(GetScore());
            mObstacleGen.SetDifficulty(mDifficulty);
            ShowLevelSign();
            ShowMenu(MENU_NONE);
            break;
        case MENUITEM_START_OVER:
            ...
            ShowMenu(MENU_NONE);
            OurActivity_ResetEncouragementToasts(0);
            break;
    }
}
